...
pv -N projects -c -f -i5 all_data.txt.gz | libdeflate-gzip -d | jq -R -r '. | fromjson | [input_line_number, .info.name, .info.version, .info.author, .info.author_email, .info.home_page,
...
echo "Creating urls"
pv -N urls -c -f -i5 all_data.txt.gz | libdeflate-gzip -d | jq -R -r '. | fromjson | .urls[] | [input_line_number, .url, .upload_time_iso_8601, .packagetype, .python_version, .requires_python, .size,
...
libdeflate-gzip -1 pypi-data.sqlite
